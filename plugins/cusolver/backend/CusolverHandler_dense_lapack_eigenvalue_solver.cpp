...
CUSOLVER_ROUTINE_HANDLER(DnGesvd_bufferSize){
        ...
        void *S;
        if (dataTypeS == CUDA_R_32F) {
            // float
            S = in->GetFromMarshal<float*>();
        } else if (dataTypeS == CUDA_R_64F) {
            // double
            S = in->GetFromMarshal<double*>();
        } else if (dataTypeS == CUDA_C_32F) {
            // cuComplex
            S = in->GetFromMarshal<float*>();
        } else if (dataTypeS == CUDA_C_64F) {
            // cuDoubleComplex
            S = in->GetFromMarshal<double*>();
        } else {
            throw "Type not supported by GVirtus!";
        ...
        void *U;
        if (dataTypeU == CUDA_R_32F) {
            // float
            U = in->GetFromMarshal<float*>();
        } else if (dataTypeU == CUDA_R_64F) {
            // double
            U = in->GetFromMarshal<double*>();
        } else if (dataTypeU == CUDA_C_32F) {
            // cuComplex
            U = in->GetFromMarshal<cuComplex*>();
        } else if (dataTypeU == CUDA_C_64F) {
            // cuDoubleComplex
        ...
        void *VT;
        if (dataTypeVT == CUDA_R_32F) {
            // float
            VT = in->GetFromMarshal<float*>();
        } else if (dataTypeVT == CUDA_R_64F) {
            // double
            VT = in->GetFromMarshal<double*>();
        } else if (dataTypeVT == CUDA_C_32F) {
            // cuComplex
            VT = in->GetFromMarshal<cuComplex*>();
        } else if (dataTypeVT == CUDA_C_64F) {
            // cuDoubleComplex
...
CUSOLVER_ROUTINE_HANDLER(DnSyevd_bufferSize){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnSyevd_bufferSize"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    ...
    cudaDataType dataTypeA = in->Get<cudaDataType>();
    int64_t lda = in->Get<int64_t>();
    cudaDataType dataTypeW = in->Get<cudaDataType>();
    cudaDataType computeType = in->Get<cudaDataType>();
    size_t workspaceInBytes;
    void *A;
    if (dataTypeA == CUDA_R_32F) {
    ...
    void *W;
    if (dataTypeW == CUDA_R_32F) {
        // float
        W = in->GetFromMarshal<float*>();
    } else if (dataTypeW == CUDA_R_64F) {
        // double
        W = in->GetFromMarshal<double*>();
    } else if (dataTypeW == CUDA_C_32F) {
        // cuComplex
        W = in->GetFromMarshal<cuComplex*>();
    } else if (dataTypeW == CUDA_C_64F) {
        // cuDoubleComplex
        W = in->GetFromMarshal<cuDoubleComplex*>();
    } else {
        throw "Type not supported by GVirtus!";
    }
    cusolverStatus_t cs;
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnSyevd_bufferSize(handle, params, jobz, uplo, n, dataTypeA, A, lda, dataTypeW, W, computeType, &workspaceInBytes);
        out->AddMarshal<size_t>(workspaceInBytes);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
        return std::make_shared<Result>(CUSOLVER_STATUS_EXECUTION_FAILED);
    }
    LOG4CPLUS_DEBUG(logger,"cusolverDnSyevd_bufferSize Executed");
    return std::make_shared<Result>(cs, out);
}

CUSOLVER_ROUTINE_HANDLER(DnSyevd){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnSyevd"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverDnParams_t params = (cusolverDnParams_t)in->Get<size_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnSyevd(handle, params, jobz, uplo, n, dataTypeA, A, lda, dataTypeW, W, computeType, pBuffer, workspaceInBytes, info);
        out->Add<void*>(W);
        out->Add<int*>(info);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
    ...
    LOG4CPLUS_DEBUG(logger,"cusolverDnSyevd Executed");
    return std::make_shared<Result>(cs,out);
}

CUSOLVER_ROUTINE_HANDLER(DnSsyevdx_bufferSize){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnSsyevdx_bufferSize"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnSsyevdx_bufferSize(handle, jobz, range, uplo, n, A, lda, vl, vu, il, iu, h_meig, W, &lwork);
        out->AddMarshal<int>(lwork);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
        return std::make_shared<Result>(CUSOLVER_STATUS_EXECUTION_FAILED);
    }
    LOG4CPLUS_DEBUG(logger,"cusolverDnSsyevdx_bufferSize Executed");
    return std::make_shared<Result>(cs, out);
}

CUSOLVER_ROUTINE_HANDLER(DnDsyevdx_bufferSize){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnDsyevdx_bufferSize"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    cusolverEigRange_t range = in->Get<cusolverEigRange_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnDsyevdx_bufferSize(handle, jobz, range, uplo, n, A, lda, vl, vu, il, iu, h_meig, W, &lwork);
        out->AddMarshal<int>(lwork);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
        return std::make_shared<Result>(CUSOLVER_STATUS_EXECUTION_FAILED);
    }
    LOG4CPLUS_DEBUG(logger,"cusolverDnDsyevdx_bufferSize Executed");
    return std::make_shared<Result>(cs, out);
}

CUSOLVER_ROUTINE_HANDLER(DnCheevdx_bufferSize){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnCheevdx_bufferSize"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    cusolverEigRange_t range = in->Get<cusolverEigRange_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnCheevdx_bufferSize(handle, jobz, range, uplo, n, A, lda, vl, vu, il, iu, h_meig, W, &lwork);
        out->AddMarshal<int>(lwork);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
        return std::make_shared<Result>(CUSOLVER_STATUS_EXECUTION_FAILED);
    }
    LOG4CPLUS_DEBUG(logger,"cusolverDnCheevdx_bufferSize Executed");
    return std::make_shared<Result>(cs, out);
}

CUSOLVER_ROUTINE_HANDLER(DnZheevdx_bufferSize){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnZheevdx_bufferSize"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    cusolverEigRange_t range = in->Get<cusolverEigRange_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnZheevdx_bufferSize(handle, jobz, range, uplo, n, A, lda, vl, vu, il, iu, h_meig, W, &lwork);
        out->AddMarshal<int>(lwork);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
        return std::make_shared<Result>(CUSOLVER_STATUS_EXECUTION_FAILED);
    }
    LOG4CPLUS_DEBUG(logger,"cusolverDnZheevdx_bufferSize Executed");
    return std::make_shared<Result>(cs, out);
}

CUSOLVER_ROUTINE_HANDLER(DnSsyevdx){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnSsyevdx"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    cusolverEigRange_t range = in->Get<cusolverEigRange_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnSsyevdx(handle, jobz, range, uplo, n, A, lda, vl, vu, il, iu, h_meig, W, work, lwork, devInfo);
        out->Add(h_meig);
        out->Add<float*>(W);
        out->Add<int*>(devInfo);
    } catch (string e){
    ...
    LOG4CPLUS_DEBUG(logger,"cusolverDnSsyevdx Executed");
    return std::make_shared<Result>(cs,out);
}

CUSOLVER_ROUTINE_HANDLER(DnDsyevdx){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnDsyevdx"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    cusolverEigRange_t range = in->Get<cusolverEigRange_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnDsyevdx(handle, jobz, range, uplo, n, A, lda, vl, vu, il, iu, h_meig, W, work, lwork, devInfo);
        out->Add(h_meig);
        out->Add<double*>(W);
        out->Add<int*>(devInfo);
    } catch (string e){
    ...
    LOG4CPLUS_DEBUG(logger,"cusolverDnDsyevdx Executed");
    return std::make_shared<Result>(cs,out);
}

CUSOLVER_ROUTINE_HANDLER(DnCheevdx){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnCheevdx"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    cusolverEigRange_t range = in->Get<cusolverEigRange_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnCheevdx(handle, jobz, range, uplo, n, A, lda, vl, vu, il, iu, h_meig, W, work, lwork, devInfo);
        out->Add(h_meig);
        out->Add<float*>(W);
        out->Add<int*>(devInfo);
    } catch (string e){
    ...
    LOG4CPLUS_DEBUG(logger,"cusolverDnCheevdx Executed");
    return std::make_shared<Result>(cs,out);
}

CUSOLVER_ROUTINE_HANDLER(DnZheevdx){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnZheevdx"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    cusolverEigRange_t range = in->Get<cusolverEigRange_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnZheevdx(handle, jobz, range, uplo, n, A, lda, vl, vu, il, iu, h_meig, W, work, lwork, devInfo);
        out->Add(h_meig);
        out->Add<double*>(W);
        out->Add<int*>(devInfo);
    } catch (string e){
    ...
    LOG4CPLUS_DEBUG(logger,"cusolverDnZheevdx Executed");
    return std::make_shared<Result>(cs,out);
}

CUSOLVER_ROUTINE_HANDLER(DnSsygvd_bufferSize){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnSsygvd_bufferSize"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnSsygvd_bufferSize(handle, itype, jobz, uplo, n, A, lda, B, ldb, W, &lwork);
        out->AddMarshal<int>(lwork);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
        return std::make_shared<Result>(CUSOLVER_STATUS_EXECUTION_FAILED);
    }
    LOG4CPLUS_DEBUG(logger,"cusolverDnSsygvd_bufferSize Executed");
    return std::make_shared<Result>(cs, out);
}

CUSOLVER_ROUTINE_HANDLER(DnDsygvd_bufferSize){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnDsygvd_bufferSize"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigType_t itype = in->Get<cusolverEigType_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnDsygvd_bufferSize(handle, itype, jobz, uplo, n, A, lda, B, ldb, W, &lwork);
        out->AddMarshal<int>(lwork);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
        return std::make_shared<Result>(CUSOLVER_STATUS_EXECUTION_FAILED);
    }
    LOG4CPLUS_DEBUG(logger,"cusolverDnDsygvd_bufferSize Executed");
    return std::make_shared<Result>(cs, out);
}

CUSOLVER_ROUTINE_HANDLER(DnChegvd_bufferSize){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnChegvd_bufferSize"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigType_t itype = in->Get<cusolverEigType_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnChegvd_bufferSize(handle, itype, jobz, uplo, n, A, lda, B, ldb, W, &lwork);
        out->AddMarshal<int>(lwork);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
        return std::make_shared<Result>(CUSOLVER_STATUS_EXECUTION_FAILED);
    }
    LOG4CPLUS_DEBUG(logger,"cusolverDnChegvd_bufferSize Executed");
    return std::make_shared<Result>(cs, out);
}

CUSOLVER_ROUTINE_HANDLER(DnZhegvd_bufferSize){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnZhegvd_bufferSize"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigType_t itype = in->Get<cusolverEigType_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnZhegvd_bufferSize(handle, itype, jobz, uplo, n, A, lda, B, ldb, W, &lwork);
        out->AddMarshal<int>(lwork);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
        return std::make_shared<Result>(CUSOLVER_STATUS_EXECUTION_FAILED);
    }
    LOG4CPLUS_DEBUG(logger,"cusolverDnZhegvd_bufferSize Executed");
    return std::make_shared<Result>(cs, out);
}

CUSOLVER_ROUTINE_HANDLER(DnSsygvd){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnSsygvd"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigType_t itype = in->Get<cusolverEigType_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnSsygvd(handle, itype, jobz, uplo, n, A, lda, B, ldb, W, work, lwork, devInfo);
        out->Add<float*>(W);
        out->Add<int*>(devInfo);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
    ...
    LOG4CPLUS_DEBUG(logger,"cusolverDnSsygvd Executed");
    return std::make_shared<Result>(cs,out);
}

CUSOLVER_ROUTINE_HANDLER(DnDsygvd){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnDsygvd"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigType_t itype = in->Get<cusolverEigType_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnDsygvd(handle, itype, jobz, uplo, n, A, lda, B, ldb, W, work, lwork, devInfo);
        out->Add<double*>(W);
        out->Add<int*>(devInfo);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
    ...
    LOG4CPLUS_DEBUG(logger,"cusolverDnDsygvd Executed");
    return std::make_shared<Result>(cs,out);
}

CUSOLVER_ROUTINE_HANDLER(DnChegvd){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnChegvd"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigType_t itype = in->Get<cusolverEigType_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnChegvd(handle, itype, jobz, uplo, n, A, lda, B, ldb, W, work, lwork, devInfo);
        out->Add<float*>(W);
        out->Add<int*>(devInfo);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
    ...
    LOG4CPLUS_DEBUG(logger,"cusolverDnChegvd Executed");
    return std::make_shared<Result>(cs,out);
}

CUSOLVER_ROUTINE_HANDLER(DnZhegvd){
    Logger logger = Logger::getInstance(LOG4CPLUS_TEXT("DnZhegvd"));
    CusolverHandler::setLogLevel(&logger);
    cusolverDnHandle_t handle = (cusolverDnHandle_t)in->Get<size_t>();
    cusolverEigType_t itype = in->Get<cusolverEigType_t>();
    cusolverEigMode_t jobz = in->Get<cusolverEigMode_t>();
    ...
    std::shared_ptr<Buffer> out = std::make_shared<Buffer>();
    try{
        cs = cusolverDnZhegvd(handle, itype, jobz, uplo, n, A, lda, B, ldb, W, work, lwork, devInfo);
        out->Add<double*>(W);
        out->Add<int*>(devInfo);
    } catch (string e){
        LOG4CPLUS_DEBUG(logger,e);
