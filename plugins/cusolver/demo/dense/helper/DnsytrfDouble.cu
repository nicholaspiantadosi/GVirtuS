...
int main(void) {
    ...
    double *dA;
    CUDA_CHECK( cudaMalloc((void**) &dA, lda * n * sizeof(double)));
    CUDA_CHECK( cudaMemcpy(dA, hA, lda * n  * sizeof(double), cudaMemcpyHostToDevice) );

    cusolverDnHandle_t handle = NULL;
    CUSOLVER_CHECK(cusolverDnCreate(&handle));

    int Lwork;
    CUSOLVER_CHECK(cusolverDnDsytrf_bufferSize(handle, n, dA, lda, &Lwork));
    ...
    CUSOLVER_CHECK(cusolverDnDsytrf(handle, CUBLAS_FILL_MODE_LOWER, n, dA, lda, devIpiv, Workspace, Lwork, devInfo));
    int hdevInfo;
    int hIpiv[n];
