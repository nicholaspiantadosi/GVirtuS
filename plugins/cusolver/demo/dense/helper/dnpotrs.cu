...
int main(void) {
    ...
    CUSOLVER_CHECK(cusolverDnPotrs(handle, params, CUBLAS_FILL_MODE_LOWER, n, ldb, CUDA_R_32F, dA, lda, CUDA_R_32F, dB, ldb, info));
    int hInfo;
    ...
    if (correct == 1) {
        printf("dnpotrs test PASSED\n");
    } else {
        printf("dnpotrs test FAILED\n");
    }
