...
int main(void) {
    ...
    size_t lwork_bytes;
    CUSOLVER_CHECK(cusolverDnZKgesv_bufferSize(handle, n, nrhs, NULL, lda, NULL, NULL, ldb, NULL, ldx, NULL, &lwork_bytes));
    lwork_bytes = 314880;
    ...
    CUSOLVER_CHECK(cusolverDnZKgesv(handle, n, nrhs, dA, lda, devIpiv, dB, ldb, dX, ldx, dWorkspace, lwork_bytes, &niter, devInfo));
    int hdevInfo;
    ...
    if (correct == 1) {
        printf("DnZKgesv test PASSED\n");
    } else {
        printf("DnZKgesv test FAILED\n");
    }
