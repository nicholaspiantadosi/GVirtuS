...
int main(void) {
    ...
    size_t lwork_bytes;
    CUSOLVER_CHECK(cusolverDnZZgels_bufferSize(handle, m, n, nrhs, NULL, lda, NULL, ldb, NULL, ldx, NULL, &lwork_bytes));
    //printf("%d\n", lwork_bytes);
    ...
    CUSOLVER_CHECK(cusolverDnZZgels(handle, m, n, nrhs, dA, lda, dB, ldb, dX, ldx, dWorkspace, lwork_bytes, &niter, devInfo));
    int hdevInfo;
    ...
    if (correct == 1) {
        printf("DnZZgels test PASSED\n");
    } else {
        printf("DnZZgels test FAILED\n");
    }
