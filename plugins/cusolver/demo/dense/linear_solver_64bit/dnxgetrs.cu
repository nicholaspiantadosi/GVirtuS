...
int main(void) {
    ...
    CUSOLVER_CHECK(cusolverDnXgetrs(handle, params, CUBLAS_OP_N, n, nrhs, CUDA_R_32F, dA, lda, devIpiv, CUDA_R_32F, dB, ldb, info));
    int hInfo;
    ...
    if (correct == 1) {
        printf("dnxgetrs test PASSED\n");
    } else {
        printf("dnxgetrs test FAILED\n");
    }
