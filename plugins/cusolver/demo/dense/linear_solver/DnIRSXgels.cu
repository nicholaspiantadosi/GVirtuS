...
int main(void) {
    ...
    cusolverDnIRSParams_t gels_irs_params = NULL;
    CUSOLVER_CHECK(cusolverDnIRSParamsCreate(&gels_irs_params));

    size_t lwork_bytes;
    CUSOLVER_CHECK(cusolverDnIRSXgels_bufferSize(handle, gels_irs_params, m, n, nrhs, &lwork_bytes));
    printf("%d\n", lwork_bytes);

    void *dWorkspace;
    cudaMalloc((void**)&dWorkspace, lwork_bytes);

    cusolverDnIRSInfos_t gels_irs_infos = NULL;
    CUSOLVER_CHECK(cusolverDnIRSInfosCreate(&gels_irs_infos));

    int *devInfo;
    int niter;
    CUDA_CHECK( cudaMalloc((void**) &devInfo, sizeof(int)));
    CUSOLVER_CHECK(cusolverDnIRSXgels(handle, gels_irs_params, gels_irs_infos, m, n, nrhs, dA, lda, dB, ldb, dX, ldx, dWorkspace, lwork_bytes, &niter, devInfo));
    int hdevInfo;
    ...
    CUSOLVER_CHECK(cusolverDnIRSInfosDestroy(gels_irs_infos));
    CUSOLVER_CHECK(cusolverDnIRSParamsDestroy(gels_irs_params));
