...
int main(void) {
    ...
    CUSOLVER_CHECK(cusolverDnGetrf_bufferSize(handle, params, m, n, CUDA_R_32F, dA, lda, CUDA_R_32F, &workspaceInBytes));
    ...
    CUSOLVER_CHECK(cusolverDnGetrf(handle, params, m, n, CUDA_R_32F, dA, lda, devIpiv, CUDA_R_32F, pBuffer, workspaceInBytes, info));
    int hInfo;
    ...
    if (correct == 1) {
        printf("dngetrf test PASSED\n");
    } else {
        printf("dngetrf test FAILED\n");
    }
