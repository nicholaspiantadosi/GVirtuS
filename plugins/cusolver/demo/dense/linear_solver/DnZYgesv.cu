...
int main(void) {
    ...
    size_t lwork_bytes;
    CUSOLVER_CHECK(cusolverDnZYgesv_bufferSize(handle, n, nrhs, NULL, lda, NULL, NULL, ldb, NULL, ldx, NULL, &lwork_bytes));
    //lwork_bytes = 88832;
    printf("%d\n", lwork_bytes);
    ...
    CUSOLVER_CHECK(cusolverDnZYgesv(handle, n, nrhs, dA, lda, devIpiv, dB, ldb, dX, ldx, dWorkspace, lwork_bytes, &niter, devInfo));
    int hdevInfo;
    ...
    if (correct == 1) {
        printf("DnZYgesv test PASSED\n");
    } else {
        printf("DnZYgesv test FAILED\n");
    }
