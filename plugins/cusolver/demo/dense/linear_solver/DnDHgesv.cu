...
int main(void) {
    ...
    size_t lwork_bytes;
    CUSOLVER_CHECK(cusolverDnDHgesv_bufferSize(handle, n, nrhs, NULL, lda, NULL, NULL, ldb, NULL, ldx, NULL, &lwork_bytes));
    lwork_bytes = 157568;
    //printf("%d\n", lwork_bytes);
    ...
    CUSOLVER_CHECK(cusolverDnDHgesv(handle, n, nrhs, dA, lda, devIpiv, dB, ldb, dX, ldx, dWorkspace, lwork_bytes, &niter, devInfo));
    int hdevInfo;
    ...
    if (correct == 1) {
        printf("DnDHgesv test PASSED\n");
    } else {
        printf("DnDHgesv test FAILED\n");
    }
