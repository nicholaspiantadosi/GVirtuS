...
int main(void) {
    ...
    float *dtau;
    CUDA_CHECK( cudaMalloc((void**) &dtau, (n - 1) * sizeof(float)));

    int Lwork;
    CUSOLVER_CHECK(cusolverDnSorgtr_bufferSize(handle, CUBLAS_FILL_MODE_UPPER, n, dA, lda, dtau, &Lwork));
    ...
    CUSOLVER_CHECK(cusolverDnSorgtr(handle, CUBLAS_FILL_MODE_UPPER, n, dA, lda, dtau, Workspace, Lwork, devInfo));
    int hdevInfo;
    CUDA_CHECK( cudaMemcpy(&hdevInfo, devInfo, sizeof(int), cudaMemcpyDeviceToHost) );
    float valuesTau[n - 1];
    CUDA_CHECK( cudaMemcpy(valuesTau, dtau, (n - 1) * sizeof(float), cudaMemcpyDeviceToHost) );
