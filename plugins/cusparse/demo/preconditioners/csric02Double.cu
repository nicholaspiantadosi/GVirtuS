...
int main(int argn, char *argv[])
{
    ...
    double hx[] = {1, 2, 3, 4};
    double hy[] = {5, 6, 7, 8};
    double hz[] = {9, 10, 11, 12};

    double hx_result[] = {1, 2, 3, 4};
    double hy_result[] = {5, 6, 7, 8};
    double hz_result[] = {9, 10, 11, 12};
    */

    const int m = 2;
    const int nnz = 3;

    double hCsrValA[] = {1, 2, 3};
    ...
    CHECK_CUDA( cudaMemcpy(dz, hz, m * sizeof(double), cudaMemcpyHostToDevice) );
    ...
    // device result check
    CHECK_CUDA( cudaMemcpy(hx, dx, m * sizeof(double), cudaMemcpyDeviceToHost) );
    CHECK_CUDA( cudaMemcpy(hy, dy, m * sizeof(double), cudaMemcpyDeviceToHost) );
    CHECK_CUDA( cudaMemcpy(hz, dz, m * sizeof(double), cudaMemcpyDeviceToHost) );
