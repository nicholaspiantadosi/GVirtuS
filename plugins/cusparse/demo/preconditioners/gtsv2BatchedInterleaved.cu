...
int main(int argn, char *argv[])
{
    ...
    float hx_result[] = {1.000000, 0.717949, 0.000000, -0.871795,
    ...
    cusparseSgtsvInterleavedBatch_bufferSizeExt(handle, algo, m, ddl, dd, ddu, dx, batchCount, &bufferSizeInBytes);

    cudaMalloc((void**)&pBuffer, bufferSizeInBytes);

    cusparseSgtsvInterleavedBatch(handle, algo, m, ddl, dd, ddu, dx, batchCount, pBuffer);
    ...
    if (correct)
        printf("gtsv2BatchedInterleaved test PASSED\n");
    else
        printf("gtsv2BatchedInterleaved test FAILED: wrong result\n");
