...
int main(int argn, char *argv[])
{
    ...
    double hx_result[] = {0.553030, 0.446970, -0.340909, 0.371212,
    ...
    cusparseDgtsv2StridedBatch_bufferSizeExt(handle, m, ddl, dd, ddu, dx, batchCount, batchStride, &bufferSizeInBytes);

    cudaMalloc((void**)&pBuffer, bufferSizeInBytes);

    cusparseDgtsv2StridedBatch(handle, m, ddl, dd, ddu, dx, batchCount, batchStride, pBuffer);
