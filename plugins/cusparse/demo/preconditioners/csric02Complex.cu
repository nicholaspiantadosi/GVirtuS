...
int main(int argn, char *argv[])
{
    ...
    cuComplex hCsrValA[] = {1, 2, 3, 4, 5, 6, 7, 8, 9};
    int hCsrRowPtrA[] = {0, 3, 4, 7, 9};
    int hCsrColIndA[] = {0, 2, 3, 1, 0, 2, 3, 1, 3};

    cuComplex hx[] = {1, 2, 3, 4};
    cuComplex hy[] = {5, 6, 7, 8};
    cuComplex hz[] = {9, 10, 11, 12};

    cuComplex hx_result[] = {1, 2, 3, 4};
    cuComplex hy_result[] = {5, 6, 7, 8};
    cuComplex hz_result[] = {9, 10, 11, 12};
    */

    const int m = 2;
    const int nnz = 3;

    cuComplex hCsrValA[] = {make_cuComplex(1, 0), make_cuComplex(2, 0), make_cuComplex(3, 0)};
    int hCsrRowPtrA[] = {0, 2, 3};
    int hCsrColIndA[] = {0, 1, 1};

    cuComplex hx[] = {make_cuComplex(1, 0), make_cuComplex(2, 0)};
    cuComplex hy[] = {make_cuComplex(3, 0), make_cuComplex(4, 0)};
    cuComplex hz[] = {make_cuComplex(5, 0), make_cuComplex(6, 0)};

    // Device memory management
    cuComplex *dCsrValA, *dx, *dy, *dz;
    ...
    CHECK_CUDA( cudaMemcpy(dz, hz, m * sizeof(cuComplex), cudaMemcpyHostToDevice) );
    ...
    cusparseCcsric02_bufferSize(handle, m, nnz,
    ...
    cusparseCcsrsv2_bufferSize(handle, trans_Lt, m, nnz,
    ...
    cusparseCcsric02_analysis(handle, m, nnz, descr_M,
    ...
    cusparseCcsrsv2_analysis(handle, trans_Lt, m, nnz, descr_L,
                             dCsrValA, dCsrRowPtrA, dCsrColIndA,
                             info_Lt, policy_Lt, pBuffer);

    // step 5: M = L * L'
    cusparseCcsric02(handle, m, nnz, descr_M,
    ...
    // step 6: solve L*z = x
    cusparseCcsrsv2_solve(handle, trans_L, m, nnz, &alpha, descr_L,
                          dCsrValA, dCsrRowPtrA, dCsrColIndA, info_L,
                          dx, dz, policy_L, pBuffer);

    // step 7: solve L'*y = z
    cusparseCcsrsv2_solve(handle, trans_Lt, m, nnz, &alpha, descr_L,
                          dCsrValA, dCsrRowPtrA, dCsrColIndA, info_Lt,
                          dz, dy, policy_Lt, pBuffer);

    // device result check
    CHECK_CUDA( cudaMemcpy(hx, dx, m * sizeof(cuComplex), cudaMemcpyDeviceToHost) );
    CHECK_CUDA( cudaMemcpy(hy, dy, m * sizeof(cuComplex), cudaMemcpyDeviceToHost) );
    CHECK_CUDA( cudaMemcpy(hz, dz, m * sizeof(cuComplex), cudaMemcpyDeviceToHost) );
