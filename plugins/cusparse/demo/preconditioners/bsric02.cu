...
int main(int argn, char *argv[])
{
    ...
    float hy_result[] = {1.526316, 0.500000, -0.105263, 0.000000};

    // Device memory management
    float *dBsrValA, *dx, *dy, *dz;
    ...
    cusparseSbsric02_bufferSize(handle, dir, mb, nnzb,
                                descr_M, dBsrValA, dBsrRowPtrA, dBsrColIndA, blockSize, info_M, &pBufferSize_M);
    cusparseSbsrsv2_bufferSize(handle, dir, trans_L, mb, nnzb,
                               descr_L, dBsrValA, dBsrRowPtrA, dBsrColIndA, blockSize, info_L, &pBufferSize_L);
    cusparseSbsrsv2_bufferSize(handle, dir, trans_Lt, mb, nnzb,
    ...
    cusparseSbsric02_analysis(handle, dir, mb, nnzb, descr_M,
    ...
    cusparseSbsrsv2_analysis(handle, dir, trans_L, mb, nnzb, descr_L,
                             dBsrValA, dBsrRowPtrA, dBsrColIndA, blockSize,
                             info_L, policy_L, pBuffer);

    cusparseSbsrsv2_analysis(handle, dir, trans_Lt, mb, nnzb, descr_L,
                             dBsrValA, dBsrRowPtrA, dBsrColIndA, blockSize,
                             info_Lt, policy_Lt, pBuffer);

    // step 5: M = L * L'
    cusparseSbsric02(handle, dir, mb, nnzb, descr_M,
    ...
    // step 6: solve L*z = x
    cusparseSbsrsv2_solve(handle, dir, trans_L, mb, nnzb, &alpha, descr_L,
                          dBsrValA, dBsrRowPtrA, dBsrColIndA, blockSize, info_L,
                          dx, dz, policy_L, pBuffer);

    // step 7: solve L'*y = z
    cusparseSbsrsv2_solve(handle, dir, trans_Lt, mb, nnzb, &alpha, descr_L,
