...
void bsrmv(cusparseHandle_t, cusparseMatDescr_t, int, int, int, int, int, int, cuComplex, cuComplex *, int *, int *, int, cuComplex *, cuComplex, cuComplex *);
...
void bsrmv(cusparseHandle_t handle, cusparseMatDescr_t descr, int m, int n, int nnz, int mb, int nb, int nnzb, cuComplex alpha, cuComplex * bsrValC, int * bsrRowPtrC, int * bsrColIndC, int blockDim, cuComplex * x, cuComplex beta, cuComplex * y)
{
    // Variabili su device
    ...
    CHECK_CUDA(cudaMemcpy(bsrValC_device, bsrValC, (blockDim * blockDim) * nnzb * sizeof(cuComplex), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(bsrRowPtrC_device, bsrRowPtrC, (mb + 1) * sizeof(int), cudaMemcpyHostToDevice));
    ...
    CHECK_CUSPARSE(cusparseCbsrmv(handle, CUSPARSE_DIRECTION_COLUMN, CUSPARSE_OPERATION_NON_TRANSPOSE, mb, nb, nnzb, &alpha, descr, bsrValC_device, bsrRowPtrC_device, bsrColIndC_device, blockDim, x_device, &beta, y_device));
