...
void bsrsv2(cusparseHandle_t, cusparseMatDescr_t, int, int, int, cuComplex *, int *, int *, int, cuComplex *, cuComplex *);
...
void bsrsv2(cusparseHandle_t handle, cusparseMatDescr_t descr, int m, int mb, int nnzb, cuComplex * bsrVal, int * bsrRowPtr, int * bsrColInd, int blockDim, cuComplex * x, cuComplex * y) {
    ...
    CHECK_CUDA(cudaMalloc((void**) &x_device, (mb*blockDim) * sizeof(cuComplex)));
    CHECK_CUDA(cudaMalloc((void**) &y_device, (mb*blockDim) * sizeof(cuComplex)));

    CHECK_CUDA(cudaMemcpy(x_device, x, m * sizeof(cuComplex), cudaMemcpyHostToDevice));
...
// step 3: query how much memory used in bsrsv2, and allocate the buffer
    cusparseCbsrsv2_bufferSize(handle, dir, trans, mb, nnzb, descr,
...
// step 4: perform analysis
    cusparseCbsrsv2_analysis(handle, dir, trans, mb, nnzb, descr,
...
// step 5: solve L*y = x
    cusparseCbsrsv2_solve(handle, dir, trans, mb, nnzb, &alpha, descr,
