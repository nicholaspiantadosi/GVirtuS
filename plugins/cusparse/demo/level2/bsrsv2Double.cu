...
void bsrsv2(cusparseHandle_t, cusparseMatDescr_t, int, int, int, double *, int *, int *, int, double *, double *);
...
void bsrsv2(cusparseHandle_t handle, cusparseMatDescr_t descr, int m, int mb, int nnzb, double * bsrVal, int * bsrRowPtr, int * bsrColInd, int blockDim, double * x, double * y) {
    ...
    CHECK_CUDA(cudaMalloc((void**) &x_device, (mb*blockDim) * sizeof(double)));
    CHECK_CUDA(cudaMalloc((void**) &y_device, (mb*blockDim) * sizeof(double)));

    CHECK_CUDA(cudaMemcpy(x_device, x, m * sizeof(double), cudaMemcpyHostToDevice));
