...
void bsrxmv(cusparseHandle_t, cusparseMatDescr_t, int, int, int, int, int, int, int, float, float*, int*, int*, int*, int*, int, float*, float, float*);
...
void bsrxmv(cusparseHandle_t handle, cusparseMatDescr_t descr, int m, int n, int nnz, int sizeOfMask, int mb, int nb, int nnzb, float alpha, float* bsrVal, int* bsrMaskPtr, int* bsrRowPtr, int* bsrEndPtr, int* bsrColInd, int blockDim, float* x, float beta, float* y)
{
    // Variabili su device
    ...
    // Allocazione variabili su device
    CHECK_CUDA(cudaMalloc((void**) &bsrVal_device, (blockDim * blockDim) * nnzb * sizeof(float)));
    ...
    CHECK_CUDA(cudaMemcpy(bsrVal_device, bsrVal, (blockDim * blockDim) * nnzb * sizeof(float), cudaMemcpyHostToDevice));
    ...
    CHECK_CUSPARSE(cusparseSbsrxmv(handle, CUSPARSE_DIRECTION_COLUMN, CUSPARSE_OPERATION_NON_TRANSPOSE, sizeOfMask, mb, nb, nnzb, &alpha, descr, bsrVal_device, bsrMaskPtr_device, bsrRowPtr_device, bsrEndPtr_device, bsrColInd_device, blockDim, x_device, &beta, y_device));
