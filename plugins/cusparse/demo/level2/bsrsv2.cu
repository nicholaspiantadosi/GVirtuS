...
void bsrsv2(cusparseHandle_t, cusparseMatDescr_t, int, int, int, float *, int *, int *, int, float *, float *);
...
void bsrsv2(cusparseHandle_t handle, cusparseMatDescr_t descr, int m, int mb, int nnzb, float * bsrVal, int * bsrRowPtr, int * bsrColInd, int blockDim, float * x, float * y) {
    ...
    CHECK_CUDA(cudaMalloc((void**) &x_device, (mb*blockDim) * sizeof(float)));
    CHECK_CUDA(cudaMalloc((void**) &y_device, (mb*blockDim) * sizeof(float)));

    CHECK_CUDA(cudaMemcpy(x_device, x, m * sizeof(float), cudaMemcpyHostToDevice));
...
// step 3: query how much memory used in bsrsv2, and allocate the buffer
    cusparseSbsrsv2_bufferSize(handle, dir, trans, mb, nnzb, descr,
...
// step 4: perform analysis
    cusparseSbsrsv2_analysis(handle, dir, trans, mb, nnzb, descr,
...
// step 5: solve L*y = x
    cusparseSbsrsv2_solve(handle, dir, trans, mb, nnzb, &alpha, descr,
