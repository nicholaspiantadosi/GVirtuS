...
int main(int argn, char *argv[])
{
    ...
    float hCscValA_result[] = {1, 5, 4, 2, 3, 9, 7, 8, 6};
    ...
    CHECK_CUDA( cudaMalloc((void**) &dCscValA,  nnz * sizeof(float)));
    ...
    cusparseSdense2csc(handle, m, n, descrA, dA, lda, dNnzPerCol, dCscValA, dCscRowIndA, dCscColPtrA);

    // device result check
    CHECK_CUDA( cudaMemcpy(hCscValA, dCscValA, nnz * sizeof(float), cudaMemcpyDeviceToHost) );
