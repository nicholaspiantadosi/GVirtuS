...
int main(int argn, char *argv[])
{
    ...
    float hBsrValC_result[] = {1, 0, 0, 4, 2, 0, 3, 0, 5, 0, 0, 8, 6, 0, 7, 9};
    int hBsrRowPtrC_result[] = {0, 2, 4};
    int hBsrColIndC_result[] = {0, 1, 0, 1};

    // Device memory management
    float *dCsrValA, *dBsrValC;
    ...
    cudaMalloc((void**)&dBsrValC, sizeof(float)*(rowBlockDim*colBlockDim)*nnzb);
    ...
    float hBsrValC[nnzb * rowBlockDim * colBlockDim];
    int hBsrRowPtrC[mb + 1];
    int hBsrColIndC[nnzb];

    // device result check
    CHECK_CUDA( cudaMemcpy(hBsrValC, dBsrValC, nnzb * rowBlockDim * colBlockDim * sizeof(float), cudaMemcpyDeviceToHost) );
