...
int main(void)
{
    ...
    cuDoubleComplex hCsrVal[] = {make_cuDoubleComplex(3, 0), make_cuDoubleComplex(2, 0), make_cuDoubleComplex(1, 0),
                           make_cuDoubleComplex(4, 0), make_cuDoubleComplex(6, 0), make_cuDoubleComplex(5, 0),
                           make_cuDoubleComplex(8, 0), make_cuDoubleComplex(9, 0), make_cuDoubleComplex(7, 0)};
    int hCsrRowPtr[] = {0, 3, 6, 9};
    int hCsrColInd[] = {2, 1, 0, 0, 2, 1, 1, 2, 0};

    cuDoubleComplex hCsrVal_sorted[nnz];
    int hCsrRowPtr_sorted[m + 1];
    int hCsrColInd_sorted[nnz];

    cuDoubleComplex hCsrVal_sorted_result[] = {make_cuDoubleComplex(1, 0), make_cuDoubleComplex(2, 0), make_cuDoubleComplex(3, 0),
                                         make_cuDoubleComplex(4, 0), make_cuDoubleComplex(5, 0), make_cuDoubleComplex(6, 0),
                                         make_cuDoubleComplex(7, 0), make_cuDoubleComplex(8, 0), make_cuDoubleComplex(9, 0)};
    int hCsrRowPtr_sorted_result[] = {0, 3, 6, 9};
    int hCsrColInd_sorted_result[] = {0, 1, 2, 0, 1, 2, 0, 1 , 2};

    cuDoubleComplex hCsrVal_unsorted[nnz];
    int hCsrRowPtr_unsorted[m + 1];
    int hCsrColInd_unsorted[nnz];

    // Device memory management
    cuDoubleComplex *dCsrVal;
    ...
    CHECK_CUSPARSE(cusparseZcsru2csr_bufferSizeExt(handle, m, n, nnz, dCsrVal, dCsrRowPtr, dCsrColInd, info, &pBufferSize));
    ...
    CHECK_CUSPARSE(cusparseZcsru2csr(handle, m, n, nnz, descrA, dCsrVal, dCsrRowPtr, dCsrColInd, info, pBuffer));

    // device result check
    CHECK_CUDA( cudaMemcpy(hCsrVal_sorted, dCsrVal, nnz * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost) );
    ...
    int correct = 1;
    for (int i = 0; i < nnz; i++) {
        if((fabs(hCsrVal_sorted[i].x - hCsrVal_sorted_result[i].x) > 0.000001)
    ...
    if (correct) {
        CHECK_CUSPARSE(cusparseZcsr2csru(handle, m, n, nnz, descrA, dCsrVal, dCsrRowPtr, dCsrColInd, info, pBuffer));

        // device result check
        CHECK_CUDA( cudaMemcpy(hCsrVal_unsorted, dCsrVal, nnz * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost) );
        ...
        correct = 1;
        for (int i = 0; i < nnz; i++) {
            if((fabs(hCsrVal_unsorted[i].x - hCsrVal[i].x) > 0.000001)
