...
int main(int argn, char *argv[])
{
    ...
    cuDoubleComplex hBsrValC_result[] = {make_cuDoubleComplex(1, 0), make_cuDoubleComplex(0, 0), make_cuDoubleComplex(0, 0), make_cuDoubleComplex(4, 0), make_cuDoubleComplex(2, 0), make_cuDoubleComplex(0, 0), make_cuDoubleComplex(3, 0), make_cuDoubleComplex(0, 0),
                                   make_cuDoubleComplex(5, 0), make_cuDoubleComplex(0, 0), make_cuDoubleComplex(0, 0), make_cuDoubleComplex(8, 0), make_cuDoubleComplex(6, 0), make_cuDoubleComplex(0, 0), make_cuDoubleComplex(7, 0), make_cuDoubleComplex(9, 0)};
    int hBsrRowPtrC_result[] = {0, 2, 4};
    int hBsrColIndC_result[] = {0, 1, 0, 1};

    // Device memory management
    cuDoubleComplex *dBsrValA, *dBsrValC;
    int *dBsrRowPtrA, *dBsrColIndA, *dBsrRowPtrC, *dBsrColIndC;

    CHECK_CUDA( cudaMalloc((void**) &dBsrValA,  nnzb * (rowBlockDimA * colBlockDimA) * sizeof(cuDoubleComplex)));
    CHECK_CUDA( cudaMalloc((void**) &dBsrRowPtrA, (mb + 1) * sizeof(int)) );
    CHECK_CUDA( cudaMalloc((void**) &dBsrColIndA, nnzb * sizeof(int)) );

    CHECK_CUDA( cudaMemcpy(dBsrValA, hBsrValA, nnzb * (rowBlockDimA * colBlockDimA) * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice) );
    ...
    cusparseZgebsr2gebsr_bufferSize(handle, dir, mb, nb, nnzb,
    ...
    cudaMalloc((void**)&dBsrValC, sizeof(cuDoubleComplex)*(rowBlockDimC*colBlockDimC)*nnzc);
    cusparseZgebsr2gebsr(handle, dir, mb, nb, nnzb,
    ...
    // device result check
    cuDoubleComplex hBsrValC[nnzc * rowBlockDimC * colBlockDimC];
    int hBsrRowPtrC[mc + 1];
    int hBsrColIndC[nnzc];

    CHECK_CUDA( cudaMemcpy(hBsrValC, dBsrValC, nnzc * (rowBlockDimC * colBlockDimC) * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost) );
