...
int main(void)
{
    ...
    CHECK_CUSPARSE(cusparseScsru2csr_bufferSizeExt(handle, m, n, nnz, dCsrVal, dCsrRowPtr, dCsrColInd, info, &pBufferSize));
    ...
    CHECK_CUSPARSE(cusparseScsru2csr(handle, m, n, nnz, descrA, dCsrVal, dCsrRowPtr, dCsrColInd, info, pBuffer));

    // device result check
    CHECK_CUDA( cudaMemcpy(hCsrVal_sorted, dCsrVal, nnz * sizeof(float), cudaMemcpyDeviceToHost) );
    ...
    if (correct) {
        CHECK_CUSPARSE(cusparseScsr2csru(handle, m, n, nnz, descrA, dCsrVal, dCsrRowPtr, dCsrColInd, info, pBuffer));

        // device result check
        CHECK_CUDA( cudaMemcpy(hCsrVal_unsorted, dCsrVal, nnz * sizeof(float), cudaMemcpyDeviceToHost) );
