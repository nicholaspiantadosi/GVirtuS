...
int main(void)
{
    ...
    cuComplex hCsrVal[] = {make_cuComplex(3, 0), make_cuComplex(2, 0), make_cuComplex(1, 0),
                           make_cuComplex(4, 0), make_cuComplex(6, 0), make_cuComplex(5, 0),
                           make_cuComplex(8, 0), make_cuComplex(9, 0), make_cuComplex(7, 0)};
    int hCsrRowPtr[] = {0, 3, 6, 9};
    int hCsrColInd[] = {2, 1, 0, 0, 2, 1, 1, 2, 0};

    cuComplex hCsrVal_sorted[nnz];
    int hCsrRowPtr_sorted[m + 1];
    int hCsrColInd_sorted[nnz];

    cuComplex hCsrVal_sorted_result[] = {make_cuComplex(1, 0), make_cuComplex(2, 0), make_cuComplex(3, 0),
                                         make_cuComplex(4, 0), make_cuComplex(5, 0), make_cuComplex(6, 0),
                                         make_cuComplex(7, 0), make_cuComplex(8, 0), make_cuComplex(9, 0)};
    int hCsrRowPtr_sorted_result[] = {0, 3, 6, 9};
    int hCsrColInd_sorted_result[] = {0, 1, 2, 0, 1, 2, 0, 1 , 2};

    cuComplex hCsrVal_unsorted[nnz];
    ...
    CHECK_CUSPARSE(cusparseCcsru2csr_bufferSizeExt(handle, m, n, nnz, dCsrVal, dCsrRowPtr, dCsrColInd, info, &pBufferSize));
    ...
    CHECK_CUSPARSE(cusparseCcsru2csr(handle, m, n, nnz, descrA, dCsrVal, dCsrRowPtr, dCsrColInd, info, pBuffer));

    // device result check
    CHECK_CUDA( cudaMemcpy(hCsrVal_sorted, dCsrVal, nnz * sizeof(cuComplex), cudaMemcpyDeviceToHost) );
    ...
    if (correct) {
        CHECK_CUSPARSE(cusparseCcsr2csru(handle, m, n, nnz, descrA, dCsrVal, dCsrRowPtr, dCsrColInd, info, pBuffer));

        // device result check
        CHECK_CUDA( cudaMemcpy(hCsrVal_unsorted, dCsrVal, nnz * sizeof(cuComplex), cudaMemcpyDeviceToHost) );
