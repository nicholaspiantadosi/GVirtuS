...
int main(int argn, char *argv[])
{
    ...
    double hCsrVal[] = {1, 2, 3, 4, 5, 6, 7, 8, 9};
    ...
    CHECK_CUDA( cudaMalloc((void**) &dCscVal, nnz * sizeof(double)) );
    ...
    cusparseStatus_t cs = cusparseCsr2cscEx2_bufferSize(handle, m, n, nnz, dCsrVal, dCsrRowPtr, dCsrColInd, dCscVal, dCscColPtr, dCscRowInd, CUDA_R_64F, CUSPARSE_ACTION_NUMERIC, CUSPARSE_INDEX_BASE_ZERO, CUSPARSE_CSR2CSC_ALG1, &bufferSize);

    cudaMalloc((void**)&buffer, bufferSize);

    cs = cusparseCsr2cscEx2(handle, m, n, nnz, dCsrVal, dCsrRowPtr, dCsrColInd, dCscVal, dCscColPtr, dCscRowInd, CUDA_R_64F, CUSPARSE_ACTION_NUMERIC, CUSPARSE_INDEX_BASE_ZERO, CUSPARSE_CSR2CSC_ALG1, buffer);

    // device result check
    CHECK_CUDA( cudaMemcpy(hCscVal, dCscVal, nnz * sizeof(double), cudaMemcpyDeviceToHost) );
