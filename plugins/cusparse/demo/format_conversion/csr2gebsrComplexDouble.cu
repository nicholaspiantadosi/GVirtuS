...
int main(int argn, char *argv[])
{
    ...
    cuDoubleComplex hCsrValA[] = {make_cuDoubleComplex(1, 0), make_cuDoubleComplex(2, 0), make_cuDoubleComplex(3, 0), make_cuDoubleComplex(4, 0), make_cuDoubleComplex(5, 0),
    ...
    cuDoubleComplex hBsrValC_result[] = {make_cuDoubleComplex(1, 0), make_cuDoubleComplex(0, 0), make_cuDoubleComplex(0, 0), make_cuDoubleComplex(4, 0),
    ...
    cuDoubleComplex *dCsrValA, *dBsrValC;
    ...
    int bufferSize;
    void *pBuffer;
    cusparseZcsr2gebsr_bufferSize(handle, dir, m, n,
    ...
    cudaMalloc((void**)&dBsrValC, sizeof(cuDoubleComplex)*(rowBlockDim*colBlockDim)*nnzb);
    cusparseZcsr2gebsr(handle, dir, m, n,
                       descrA,
    ...
    cuDoubleComplex hBsrValC[nnzb * rowBlockDim * colBlockDim];
    int hBsrRowPtrC[mb + 1];
    int hBsrColIndC[nnzb];

    // device result check
    CHECK_CUDA( cudaMemcpy(hBsrValC, dBsrValC, nnzb * rowBlockDim * colBlockDim * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost) );
