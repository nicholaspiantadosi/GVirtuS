...
int main(int argn, char *argv[])
{
    ...
    cuComplex hCscVal[nnz];
    int hCscColPtr[n + 1];
    int hCscRowInd[nnz];

    cuComplex hCscVal_result[] = {make_cuComplex(1, 0), make_cuComplex(5, 0), make_cuComplex(4, 0), make_cuComplex(8, 0), make_cuComplex(2, 0), make_cuComplex(6, 0), make_cuComplex(3, 0), make_cuComplex(7, 0), make_cuComplex(9, 0)};
    int hCscColPtr_result[] = {0, 2, 4, 6, 9};
    int hCscRowInd_result[] = {0, 2, 1, 3, 0, 2, 0, 2, 3};

    // Device memory management

    cuComplex *dCsrVal, *dCscVal;
    ...
    CHECK_CUDA( cudaMalloc((void**) &dCscVal, nnz * sizeof(cuComplex)) );
    ...
    cusparseStatus_t cs = cusparseCsr2cscEx2_bufferSize(handle, m, n, nnz, dCsrVal, dCsrRowPtr, dCsrColInd, dCscVal, dCscColPtr, dCscRowInd, CUDA_C_32F, CUSPARSE_ACTION_NUMERIC, CUSPARSE_INDEX_BASE_ZERO, CUSPARSE_CSR2CSC_ALG1, &bufferSize);

    cudaMalloc((void**)&buffer, bufferSize);

    cs = cusparseCsr2cscEx2(handle, m, n, nnz, dCsrVal, dCsrRowPtr, dCsrColInd, dCscVal, dCscColPtr, dCscRowInd, CUDA_C_32F, CUSPARSE_ACTION_NUMERIC, CUSPARSE_INDEX_BASE_ZERO, CUSPARSE_CSR2CSC_ALG1, buffer);

    // device result check
    CHECK_CUDA( cudaMemcpy(hCscVal, dCscVal, nnz * sizeof(cuComplex), cudaMemcpyDeviceToHost) );
