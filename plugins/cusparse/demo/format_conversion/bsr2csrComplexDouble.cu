...
int main(int argn, char *argv[])
{
    ...
    CHECK_CUDA( cudaMalloc((void**) &dBsrValA,  nnzb * (blockDim * blockDim) * sizeof(cuDoubleComplex)));
    ...
    CHECK_CUDA( cudaMemcpy(dBsrValA, hBsrValA, nnzb * (blockDim * blockDim) * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice) );
    ...
    CHECK_CUDA( cudaMemcpy(dCsrValC, hCsrValC, nnz * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice) );
    ...
    cusparseZbsr2csr(handle, dir, mb, mb,
                     descrA,
