...
int main(int argn, char *argv[])
{
    ...
    double hBsrValC_result[] = {1, 0, 0, 4, 2, 0, 3, 0, 5, 0, 0, 8, 6, 0, 7, 4};
    int hBsrRowPtrC_result[] = {0, 2, 4};
    int hBsrColIndC_result[] = {0, 1, 0, 1};

    // Device memory management
    double *dBsrValA, *dBsrValC;
    int *dBsrRowPtrA, *dBsrColIndA, *dBsrRowPtrC, *dBsrColIndC;

    CHECK_CUDA( cudaMalloc((void**) &dBsrValA,  nnzb * (rowBlockDimA * colBlockDimA) * sizeof(double)));
    CHECK_CUDA( cudaMalloc((void**) &dBsrRowPtrA, (mb + 1) * sizeof(int)) );
    CHECK_CUDA( cudaMalloc((void**) &dBsrColIndA, nnzb * sizeof(int)) );

    CHECK_CUDA( cudaMemcpy(dBsrValA, hBsrValA, nnzb * (rowBlockDimA * colBlockDimA) * sizeof(double), cudaMemcpyHostToDevice) );
    ...
    cusparseDgebsr2gebsr_bufferSize(handle, dir, mb, nb, nnzb,
    ...
    cudaMalloc((void**)&dBsrValC, sizeof(double)*(rowBlockDimC*colBlockDimC)*nnzc);
    cusparseDgebsr2gebsr(handle, dir, mb, nb, nnzb,
    ...
    // device result check
    double hBsrValC[nnzc * rowBlockDimC * colBlockDimC];
    int hBsrRowPtrC[mc + 1];
    int hBsrColIndC[nnzc];

    CHECK_CUDA( cudaMemcpy(hBsrValC, dBsrValC, nnzc * (rowBlockDimC * colBlockDimC) * sizeof(double), cudaMemcpyDeviceToHost) );
