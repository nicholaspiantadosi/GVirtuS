...
int main(){
    ...
    int *csrRowPtrX;
    int *csrColIndX;
    CHECK_CUDA( cudaMalloc((void**) &csrValX, sizeof(float) * total_nnz) );
    CHECK_CUDA( cudaMalloc((void**) &csrRowPtrX, sizeof(int) * (m+1))) ;
    CHECK_CUDA( cudaMalloc((void**) &csrColIndX, sizeof(int) * total_nnz)) ;

    float hCsrVal[] = {1, 3, -4, 5, 2, 7, 8, 6, 9, 3.5, 5.5, 6.5, -9.9};
    int hCsrRowPtrX[] = {0, 2, 4, 7, 9, 11, 13};
    int hCsrColIndX[] = {0, 1, 1, 2, 0, 3, 4, 2, 4, 3, 4, 0, 2};

    CHECK_CUDA( cudaMemcpy(csrValX, hCsrVal, total_nnz * sizeof(float), cudaMemcpyHostToDevice) );
    ...
    CHECK_CUSPARSE( cusparseSnnz_compress(handle, m, descrX, csrValX,
                                         csrRowPtrX, nnzPerRowY,
                                         &testNNZTotal, tol));
    float *csrValY;
    int *csrRowPtrY;
    int *csrColIndY;
    CHECK_CUDA( cudaMalloc((void**) &csrValY, sizeof(float) * testNNZTotal));
    CHECK_CUDA( cudaMalloc((void**) &csrRowPtrY, sizeof(int) * (m+1)));
    CHECK_CUDA( cudaMalloc((void**) &csrColIndY, sizeof(int) * testNNZTotal));

    CHECK_CUSPARSE( cusparseScsr2csr_compress( handle, m, n, descrX, csrValX,
    ...
    int hNnzPerRowY[m];
    float hCsrValY[testNNZTotal];
    int hCsrRowPtrY[m + 1];
    int hCsrColIndY[testNNZTotal];

    CHECK_CUDA( cudaMemcpy(hNnzPerRowY, nnzPerRowY, m * sizeof(int), cudaMemcpyDeviceToHost) );
    CHECK_CUDA( cudaMemcpy(hCsrValY, csrValY, testNNZTotal * sizeof(float), cudaMemcpyDeviceToHost) );
