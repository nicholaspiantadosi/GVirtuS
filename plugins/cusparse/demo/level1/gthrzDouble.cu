...
void gthrz(cusparseHandle_t, int, int, double *, double *, int *);
...
void gthrz(cusparseHandle_t handle, int n, int nnz, double * y, double * xVal, int * xInd) {
    ...
    CHECK_CUSPARSE(cusparseDgthrz(handle, nnz, y_device, xVal_device, xInd_device, CUSPARSE_INDEX_BASE_ZERO));
