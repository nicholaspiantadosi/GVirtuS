...
void sctr(cusparseHandle_t, int, int, double *, int *, double *);
...
void sctr(cusparseHandle_t handle, int n, int nnz, double * xVal, int * xInd, double * y) {
    ...
    CHECK_CUSPARSE(cusparseDsctr(handle, nnz, xVal_device, xInd_device, y_device, CUSPARSE_INDEX_BASE_ZERO));
