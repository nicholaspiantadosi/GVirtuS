...
void gthrz(cusparseHandle_t, int, int, float *, float *, int *);
...
void gthrz(cusparseHandle_t handle, int n, int nnz, float * y, float * xVal, int * xInd) {
    ...
    CHECK_CUSPARSE(cusparseSgthrz(handle, nnz, y_device, xVal_device, xInd_device, CUSPARSE_INDEX_BASE_ZERO));
