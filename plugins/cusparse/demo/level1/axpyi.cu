...
void axpyi(cusparseHandle_t, int, int, float *, float *, int *, float *);
...
int main(void)
{
    ...
    //CHECK_CUSPARSE(cusparseSaxpyi(handle, nnz, &alpha, xVal, xInd, y_result, CUSPARSE_INDEX_BASE_ZERO));
...
void axpyi(cusparseHandle_t handle, int n, int nnz, float * alpha, float * xVal, int * xInd, float * y) {
    ...
    CHECK_CUSPARSE(cusparseSaxpyi(handle, nnz, alpha, xVal_device, xInd_device, y_device, CUSPARSE_INDEX_BASE_ZERO));
