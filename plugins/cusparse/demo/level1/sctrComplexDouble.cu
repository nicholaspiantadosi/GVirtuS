...
void sctr(cusparseHandle_t, int, int, cuDoubleComplex *, int *, cuDoubleComplex *);
...
int main(void)
{
    ...
    initializeArrayRandomSparsecuDoubleComplex(vector, n, nnz);
...
void sctr(cusparseHandle_t handle, int n, int nnz, cuDoubleComplex * xVal, int * xInd, cuDoubleComplex * y) {
    ...
    CHECK_CUSPARSE(cusparseZsctr(handle, nnz, xVal_device, xInd_device, y_device, CUSPARSE_INDEX_BASE_ZERO));
