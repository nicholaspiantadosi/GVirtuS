...
void sctr(cusparseHandle_t, int, int, cuComplex *, int *, cuComplex *);
...
int main(void)
{
    ...
    initializeArrayRandomSparsecuComplex(vector, n, nnz);
...
void sctr(cusparseHandle_t handle, int n, int nnz, cuComplex * xVal, int * xInd, cuComplex * y) {
    ...
    CHECK_CUSPARSE(cusparseCsctr(handle, nnz, xVal_device, xInd_device, y_device, CUSPARSE_INDEX_BASE_ZERO));
