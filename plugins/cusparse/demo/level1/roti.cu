...
void roti(cusparseHandle_t , int , int , float * , int * , float * , float * , float * );
...
void roti(cusparseHandle_t handle, int n, int nnz, float * xVal, int * xInd, float * y, float * c, float * s) {
    ...
    CHECK_CUSPARSE(cusparseSroti(handle, nnz, xVal_device, xInd_device, y_device, c, s, CUSPARSE_INDEX_BASE_ZERO));
