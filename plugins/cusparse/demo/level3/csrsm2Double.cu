...
int main(int argn, char *argv[])
{
    ...
    CHECK_CUDA( cudaMalloc((void**) &dB,  m * nrhs * sizeof(double)));
    CHECK_CUDA( cudaMalloc((void**) &dC,  m * nrhs * sizeof(double)));
    ...
    CHECK_CUDA( cudaMemcpy(dB, hB, m * nrhs * sizeof(double), cudaMemcpyHostToDevice) );
    CHECK_CUDA( cudaMemcpy(dC, hC, m * nrhs * sizeof(double), cudaMemcpyHostToDevice) );
    ...
    cusparseDcsrsm2_bufferSizeExt(handle, algo, CUSPARSE_OPERATION_NON_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE, m, nrhs, nnz,  &alpha, descrA,
                               dCsrValA, dCsrRowPtrA, dCsrColIndA, dB, ldb, info, CUSPARSE_SOLVE_POLICY_NO_LEVEL, &pBufferSize);

    cudaMalloc((void**)&pBuffer, pBufferSize);

    cusparseDcsrsm2_analysis(handle, algo, CUSPARSE_OPERATION_NON_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE, m, nrhs, nnz, &alpha, descrA,
    ...
    // WHERE IS C???
    cusparseDcsrsm2_solve(handle, algo, CUSPARSE_OPERATION_NON_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE, m, nrhs, nnz, &alpha, descrA,
                          dCsrValA, dCsrRowPtrA, dCsrColIndA, dB, ldb, info, CUSPARSE_SOLVE_POLICY_NO_LEVEL, pBuffer);
    ...
    // device result check
    CHECK_CUDA( cudaMemcpy(hC, dC, m * nrhs * sizeof(double), cudaMemcpyDeviceToHost) );
